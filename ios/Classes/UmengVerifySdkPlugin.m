...
- (void)setupChannelWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar;
...
- (BOOL)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result registrar: (NSObject<FlutterPluginRegistrar>*)registrar{
    ...
    if ([@"getVerifyVersion" isEqualToString:call.method]){
        ...
        UIViewController *vc = [self um_topViewController];
...
-(UMCustomModel *)getUMCustomModel:(NSDictionary *)dic
{
    ...
    if ([[dic allKeys] containsObject:@"loginBtnBgImgs"]) {
        ...
        if ([arr count]==3) {
            UIImage *img0 = [UIImage imageNamed:arr[0]];
            UIImage *img1 = [UIImage imageNamed:arr[1]];
            UIImage *img2 = [UIImage imageNamed:arr[2]];
            if (img0 && img1 && img2) {
                model.loginBtnBgImgs = @[img0, img1, img2];
            }
        }
    ...
    if ([[dic allKeys] containsObject:@"checkBoxImages"]) {
        ...
        if ([arr count]==2) {
            ...
            UIImage *img1 = [UIImage imageNamed:arr[1]];
            if (img0 && img1) {
                model.checkBoxImages = @[img0, img1];
            }
        }
...
- (UIViewController *)um_topViewController {
...
static NSObject<FlutterPluginRegistrar>* _um_registrar;
static UMengflutterpluginForVerify* _verifyHandler;
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    ...
    _verifyHandler = [[UMengflutterpluginForVerify alloc] init];
    [_verifyHandler setupChannelWithRegistrar:registrar];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    BOOL resultCode = [_verifyHandler handleMethodCall:call result:result registrar:_um_registrar];
